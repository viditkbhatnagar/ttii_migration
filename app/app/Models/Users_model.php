...
class Users_model extends Base_model
{
    ...
    protected $allowedFields = ['user_id','name', 'email', 'phone', 'role_id', 'otp', 'password','user_designation_id'];  // Fields that can be manipulated
    ...
    // Login
    public function login($email, $password, $role_id=null){
        $user_check = $this->get(['user_email' => $email, 'role_id' => $role_id]);
    ...
    //   $phone_full = $code.$phone;
      $phone_full = $phone; //NEW
        $user_check = $this->get(['phone' => $phone_full]);
    ...
    public function login_phone_password($code, $phone,$password,$device_id){
        $phone_full = $code.$phone;
        $user_check = $this->get(['phone' => $phone]);
    ...
    public function userdata($user){
        ...
        $userdata['user_image'] = valid_file($user->profile_picture) ? base_url(get_file($user->profile_picture)) : base_url('uploads/dummy.jpg');
    ...
    public function update_password($email, $password){
        $user = $this->get(['email' => $email])->getRow();
        if (!$user) {
            $user = $this->get(['user_email' => $email])->getRow();
        }
        if ($user) {
            $this->edit(['password' => $password], ['id' => $user->id]);
            return true;
        }
        return false;
    }
