...
<script type="text/javascript">
    function show_small_modal(url, header)
    {
        ...
        $('#small-modal-content').html('<div style="padding:20px; text-align:center;"><img src="<?=base_url('assets/')?>loading.gif" width="100"></div>');
    ...
    function show_ajax_modal(url, header)
    {
        ...
        // SHOWING AJAX PRELOADER IMAGE
        $('#ajax-modal-content').html('<div style="padding:20px; text-align:center;"><img src="<?=base_url('assets/')?>loading.gif" width="100"></div>');
    ...
    {
        // SHOWING AJAX PRELOADER IMAGE
        $('#large-modal-content').html('<div style="padding:20px; text-align:center;"><img src="<?=base_url('assets/')?>loading.gif" width="100"></div>');
    ...
    {
        // SHOWING AJAX PRELOADER IMAGE
        $('#full-modal-content').html('<div style="padding:20px; text-align:center;"><img src="<?=base_url('assets/')?>loading.gif" width="100"></div>');
  ...
  <tbody>
    ...
    <?php foreach($submissions as $i => $s): ?>
      <tr>
        ...
        <td><?= date('d M Y h:i A', strtotime($s['submitted_time'])) ?></td>
        <td><?= $s['marks'] ?></td>
        <td><?= $s['remarks'] ?></td>
        <?php
        $files = !empty($s['assignment_files']) ? json_decode($s['assignment_files'], true) : [];

        if (!empty($files) && is_array($files)) {
            $file_url = base_url(get_file($files[0]));
        } else {
            $file_url = '#';
        }
        ?>
        <td><button class="btn btn-sm btn-success" onclick="show_ajax_modal('<?=base_url('admin/assignment/ajax_edit_remarks/'.$s['submission_id'])?>', 'Edit Remarks')">Change mark & remarks</button>
        </td>
        <td>
            ...
            <?php else: ?>
            <a href="#" class="btn btn-sm btn-outline-primary rounded-pill">No File Found</a>
            <?php endif; ?>
            <br><br>

            <button class="btn btn-sm btn-outline-danger rounded-pill" onclick="delete_modal('<?=base_url('admin/assignment/delete_submitted_file/'.$s['submission_id'])?>')">Delete file</button>
            </td>
      </tr>
    <?php endforeach; ?>
    <?php else: ?>
