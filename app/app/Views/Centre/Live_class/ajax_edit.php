...
<form autocomplete="off" action="<?=base_url('centre/live_class/edit/'.$edit_data['id'])?>" method="post" enctype="multipart/form-data">
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <div class="live-preview">
                        <div class="row gy-4">
                             ...
                             <div class="col-lg-6 p-2">
                                <div>
                                    <label for="session_id" class="form-label">Session ID<span class="required text-danger">*</span></label>
                                    <input type="text" class="form-control" id="session_id" name="session_id" value="<?=$edit_data['session_id']?>" required readonly>
                                </div>
                            </div> 
                            ...
                            <div class="col-lg-6">
                                <div class="mt-3">
                                    <label class="form-label mb-0">Date<span class="required text-danger">*</span></label>
                                    <input type="date" class="form-control" name="date" value="<?=$edit_data['date']?>" required>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-6 p-2">
                             ...
                             <div class="col-lg-12 mt-3">
                                <input type="checkbox" id="is_repetitive" name="is_repetitive" <?php if($edit_data["is_repetitive"]==1)echo "checked"; ?>>
                                <label for="is_repetitive">Is Repetitive?</label>
                            </div>
                            
                            <?php
                            if(!empty($edit_data["repeat_dates"]))
                            {
                                
                                $dates = json_decode($edit_data["repeat_dates"]);
...
$(document).ready(function() {
    ...
    function get_course(category_id){
     ...
     $.ajax({
                url: '<?php echo base_url("Centre/Live_class/get_course"); ?>',
                type: 'POST',
    ...
    function get_package(course_id){
     ...
     $.ajax({
                url: '<?php echo base_url("Centre/Live_class/get_package"); ?>',
                type: 'POST',
     ...
     $.ajax({
                url: '<?php echo base_url("Centre/Live_class/get_instructor"); ?>',
                type: 'POST',
                data: { course_id: course_id },
                success: function(data) {
                    // Append HTML options to select element
