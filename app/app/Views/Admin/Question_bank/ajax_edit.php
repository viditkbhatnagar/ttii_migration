...
<form  action="<?=base_url('admin/question_bank/edit/'.$edit_data['id'])?>" method="post"  enctype="multipart/form-data">
    <input type="hidden" name="question_type" value="mcq">
    <input type="hidden" name="type" value="1">
    <input type="hidden" name="is_equation" value="<?=$edit_data['is_equation']?>">
    <input type="hidden" name="is_equation_solution" value="<?=$edit_data['is_equation_solution']?>">
    <input type="hidden" name="number_of_options" value="<?=$edit_data['number_of_options']?>">
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <div class="live-preview">
                        <div class="row gy-4">
                            ...
                            <div class="col-lg-12 p-2 question-title">
                                ...
                                <textarea class="form-textarea editor" name="title" id="editor1"><?=($edit_data['is_equation']==1) ? '' : $edit_data['title']?></textarea>
                            </div>
                            ...
                            <div class="col-lg-12 p-2">
                                ...
                                <textarea class="form-textarea editor" name="hint" id="editor2"><?=$edit_data['hint']?></textarea>
                            </div>
                            <div class="col-lg-12 p-2">
                                <div>
                            ...
                            <?php if($edit_data['is_equation_solution']==1){ ?>
                                <div class="col-lg-12 p-2">
                                    <div>
                                        <label for="name" class="form-label">Solution equation</label>
                                        <input type="text" class="form-control" name="solution_equation" value="<?=($edit_data['is_equation_solution']==1) ? $edit_data['solution'] : ''?>">
                                    </div>
                                </div>
                            <?php }else{ ?>   
                                <div class="col-lg-12 p-2 solution">
                                    <label for="title" class="form-label">Solution</label>
                                    <textarea class="form-textarea editor" name="solution" id="editor3"><?=($edit_data['is_equation_solution']==1) ?  : $edit_data['solution']?></textarea>
                                </div>
                            <?php } ?>    
                            <div class="col-lg-12 p-2">
                                <div>
                            ...
                            <?php
                            $options = json_decode($edit_data['options']); 
                            if(!empty($options))
                            {
                                $decoded_answer = json_decode($edit_data['correct_answers']);
                                $final_answer = $decoded_answer[0] - 1;
                                // log_message('error','$final_answer '.print_r($final_answer,true));
                                ?>
                                <?php foreach($options as $key => $option) { ?>
                                    
                                    <div class="col-lg-12 p-2 question-title">
                                         <input type="checkbox" name="correct[]" value="<?=$key+1?>" <?=($key==$final_answer) ? 'checked' : ''?>> Is this the answer?
                                        <textarea class="form-control editor" name="option[]" id="editor"><?=strip_tags($option)?></textarea>
                                    </div>
                                <?php } 
