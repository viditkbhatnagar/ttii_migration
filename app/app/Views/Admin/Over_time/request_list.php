                    ...
                    <div class="custom-dropdown">
                        <select class="form-control" name="user_id">
                            ...
                            <?php foreach($users as $user){ ?>
                                <option value="<?=$user['id']?>" <?php echo isset($_GET['user_id']) && $_GET['user_id'] == $user['id'] ? 'selected' : ''; ?>><?=$user['name']?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                                ...
                                <?php foreach($over_time_data as $key=> $over_time) { ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <button class="btn btn-success addMembers-modal" onclick="show_ajax_modal('<?=base_url('app/over_time/ajax_approve/'.$over_time['id'])?>', 'Approve Over Time')">
                                                <i class="ri-tick-fill me-1 align-bottom"></i>
                                                Change Status
                                            </button>
                                            <?php } ?>
                                        </td>
