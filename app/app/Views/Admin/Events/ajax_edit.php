<form autocomplete="off" action="<?= base_url('admin/events/edit/'.$edit_data['id']) ?>" method="post" enctype="multipart/form-data">
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <div class="live-preview">
                        <div class="row">
                            ...
                            <div class="col-lg-6 p-2">
                                ...
                                <select class="form-control" name="instructor_id">
                                    ...
                                    <?php foreach($instructors as $instructor){ ?>
                                        <option value="<?=$instructor['id']?>" <?=$edit_data['instructor_id']==$instructor['id'] ? 'selected' : '' ?>><?=$instructor['name']?></option>
                                    <?php } ?>    
                                </select>
                            </div>
                            ...
                            <div class="col-lg-6 p-2">
                                ...
                                <input type="date" class="form-control" id="event_date" name="event_date" required value="<?=$edit_data['event_date']?>">
                            </div>
                            ...
                            <div class="col-lg-6 p-2">
                                ...
                                <input type="text" class="form-control" id="duration" name="duration" required readonly value="<?=$edit_data['duration']?>">
                            </div>
                                ...
                                <div class="mt-3">
                                    <div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" value="1" name="is_recording_available" id="defaultIndeterminateCheck1" <?php if($edit_data["is_recording_available"]==1)echo "checked"; ?>>
...
<?php
$edit_data['objectives'] = json_decode($edit_data['objectives']);
?>
                            <!-- Number of Objectives -->
                            <div class="col-lg-6 p-2">
                                <label for="num_objectives" class="form-label">Number of Objectives</label>
                                <input type="number" class="form-control" id="num_objectives" name="num_objectives" min="1" value="<?=$edit_data['num_objectives']?>" placeholder="Enter number of objectives">
                            </div>
                            <div class="col-lg-6 p-2">
                            ...
                            <!-- Objectives -->
                            <div class="col-lg-6 p-2" id="objectives_container">
                                <?php if (!empty($edit_data['objectives'])): ?>
                                    <?php foreach ($edit_data['objectives'] as $index => $objective): ?>
                                        <div class="p-2">
                                            <label for="objective_<?= $index + 1 ?>" class="form-label">Objective <?= $index + 1 ?></label>
                                            <input type="text" class="form-control" id="objective_<?= $index + 1 ?>" name="objectives[]" placeholder="Enter objective <?= $index + 1 ?>" value="<?= htmlspecialchars($objective) ?>">
                                        </div>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </div>
...
$(document).ready(function () {
    ...
    // Function to generate objectives dynamically
    function generateObjectives(numObjectives) {
        ...
        // Pre-fill the objectives if they exist in the form (for edit page)
        const existingObjectives = <?= json_encode($edit_data['objectives']) ?>; // PHP array of objectives
        if (existingObjectives && existingObjectives.length > 0) {
            existingObjectives.forEach((objective, index) => {
                const objectiveInput = document.getElementById(`objective_${index + 1}`);
                if (objectiveInput) {
    ...
    // Trigger the generateObjectives function when the button is clicked
    document.getElementById('generate_objectives').addEventListener('click', function () {
        ...
        generateObjectives(numObjectives);
    });
    
    // Trigger the generate objectives based on existing value of num_objectives on page load
    const numObjectives = document.getElementById('num_objectives').value;
    if (numObjectives) {
        generateObjectives(numObjectives);
    }
