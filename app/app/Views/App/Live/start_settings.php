<?php 
    // $user = $this->db->get_where('users', ['id' => $_SESSION['user_id']])->row_array();
    ...
    function websdkready() {
        ...
        <?php
            if (($live_class)){
                ...
                var JOIN_URL = "<?=base_url('app/live/meeting/')?>";
                var SIGNATURE_URL = "<?=base_url('app/live/generate_signature')?>";
                var USER_NAME = "<?=base64_encode($user['name'])?>"; // BASE 64 ENCODED
        ...
        function generateSignature() {
            ...
            fetch(SIGNATURE_URL + "?meeting_number=" + encodeURIComponent(MEETING_NUMBER) + "&role=" + encodeURIComponent(ROLE))
                ...
                .then(function (res) {
                    ...
                    var joinUrl = JOIN_URL + "?preview=<?=$_GET['preview'] ?? 0?>&" + testTool.serialize(meeting_config);
