...
class Counsellor extends AppBaseController
{
    ...
    public function index(){
        ...
        $this->data['list_items'] = $this->users_model->get(['role_id'=>9])->getResultArray();
        
        $this->data['page_title'] = 'Counsellor';
        ...
        if ($this->request->getMethod() === 'post'){
            ...
            if($check_phone_duplication == 0 && $check_email_duplication == 0) 
            {
                ...
                if (!empty($croppedImage)) {
                    $image = $this->upload_base64_image('counsellor', $croppedImage);
                }
                ...
                $counsellor = $this->users_model->add($data);
                if ($counsellor){
    ...
    public function edit($id){
        if ($this->request->getMethod() === 'post'){
            ...
            $phone = $this->request->getPost('phone');
            $email = $this->request->getPost('email');
            $check_phone_duplication = $this->users_model->get(['country_code' => $code ,'phone' => $phone,'id !=' => $id])->getNumRows();
            ...
            if($check_phone_duplication == 0 && $check_email_duplication == 0) 
            {
                ...
                if($this->request->getPost('password') != ''){
                    $data['password']= $this->users_model->password_hash($this->request->getPost('password'), PASSWORD_DEFAULT);
                    
                }

                $counsellor = $this->users_model->edit($data, ['id' => $id]);
                if ($counsellor){
